...
class MNIST_OD_Trainer:
    def __init__(self, sess):
        ...
        self.batch_size = 16
        self.pos_net_ratio = 1 #:1
        self.min_conf = 0.4

        self.sub_im_size = [28, 28]
        self.im_size = [128, 128]
        self.show_size = (256, 256)
        ...
        self.im_batch_loading = []
        self.bb_batch_loading = []
        self.enc_bb_batch_loading = [[],[],[]]
    ...
    def load_batch(self):
        ...
        self.im_batch = np.asarray(im_batch)
        self.bb_batch = np.asarray(bb_batch)
        self.enc_bb_batch = enc_bb_batch
    ...
    def load_single(self, index):
        ...
        self.im_batch_loading[index] = im
        self.bb_batch_loading[index] = bbs
        self.enc_bb_batch_loading[0][index] = encoded_bbs8
        self.enc_bb_batch_loading[1][index] = encoded_bbs16
        self.enc_bb_batch_loading[2][index] = encoded_bbs32

    def parallel_load_batch(self):
        self.im_batch_loading = [None] * self.batch_size
        self.bb_batch_loading = [None] * self.batch_size
        self.enc_bb_batch_loading = [[None] * self.batch_size, [None] * self.batch_size, [None] * self.batch_size]
        
        loaders = []
        for i in range(self.batch_size):
            loader = threading.Thread(target=self.load_single, args=(i,))
            loader.start()
            loaders.append(loader)
        for loader in loaders:
            loader.join()

        self.enc_bb_batch_loading[0] = np.asarray(self.enc_bb_batch_loading[0])
        self.enc_bb_batch_loading[1] = np.asarray(self.enc_bb_batch_loading[1])
        self.enc_bb_batch_loading[2] = np.asarray(self.enc_bb_batch_loading[2])

        self.im_batch = np.asarray(self.im_batch_loading)
        self.bb_batch = np.asarray(self.bb_batch_loading)
        self.enc_bb_batch = self.enc_bb_batch_loading

    def copy_batch(self):
        return self.im_batch[:], self.bb_batch[:], self.enc_bb_batch[:]

    def thread_activity(self, dataset_type = "Train"):
        if not self.STOP:
            self.STOP = True
            self.parallel_load_batch()
            self.STOP = False
            ...
            for b in range(self.num_bb): # for each of the 3 aspect ratios
                for d_bb in self.default_bb[scale][b]: # for each bounding box (each pixel)
                    ...
                    if iou > 0.5:
                        ...
                        if gt[p_y, p_x, b*6 + 1] != 1.0:
                            gt[p_y, p_x, b*6 + 0] = 1.0 # is class 0 (background)

                    # print p_y, p_x, b, gt[p_y, p_x, b*6:b*6+2]
    ...
    def loss(self, net_image, gt_image, scale):
        ...
        num_pos = tf.cast(tf.reduce_sum(pos_mask,axis=1), tf.int32)
        num_neg = tf.reduce_max(tf.minimum(tf.maximum(num_pos * self.pos_net_ratio, 1), vec_len)) # take at least 3 negative
        print num_pos
        print num_neg
        ...
        pos_loss = tf.reduce_mean((4*tf.reduce_sum(pos_xe_vec, axis=1) + tf.reduce_sum(pos_bb_vec, axis=1)) / tf.cast(tf.maximum(num_pos, 1), tf.float32))
        print pos_loss

        neg_loss = tf.reduce_mean((4*tf.reduce_sum(top_neg_xe_vec, axis=1)) /  tf.cast(tf.maximum(num_neg, 1), tf.float32))
        print neg_loss

        self.debug[scale] = tf.cast(num_pos, tf.float32)
    ...
    def train(self):
        ...
        batch_getter = threading.Thread(target=self.thread_activity, args=())
        batch_getter.start()
        for i in range(train_iterations):
            ...
            if self.STOP:
                ...
                break
            ims, bbs, enc_bbs = self.copy_batch()
            ...
            if i % 100 == 0 or i == train_iterations-1:
                ...
                cv2.waitKey(10)
                cv2.imwrite("od_ims/" + str(i).zfill(9) + ".png", show_im2)
...
# 
        ...
        show_im = (show_im * 255).astype(np.uint8)
        return show_im
